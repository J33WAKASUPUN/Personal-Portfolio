...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const fetchUserProfile = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch('https://jeewakadev.duckdns.org/auth/profile', {
        headers: {
  ...
  const login = async (email: string, password: string) => {
    const response = await fetch('https://jeewakadev.duckdns.org/auth/login', {
      method: 'POST',
      headers: {
  ...
  const verifyPattern = async (tempToken: string, pattern: string) => {
    const response = await fetch('https://jeewakadev.duckdns.org/auth/verify-pattern', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
